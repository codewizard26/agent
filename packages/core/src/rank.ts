 ...
 */
export function profileBrief(profile: ParsedProfile): string {
  return [
...
export function jobBrief(job: NormalizedJob, descriptionChars = 2000): string {
  return [
...
export async function rankJobs(
  ...
  if (failures.length > 0) {
    console.warn(
      `ranking: ${failures.length}/${batches.length} batches failed — ` +
