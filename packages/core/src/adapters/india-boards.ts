...
function splitRestrictions(value: string | undefined): string[] {
  if (!value) return [];
  return value
    .split(/[,/]|\bor\b/i)
    .map((v) => v.trim())
...
export function normalizeRemotive(raw: RemotiveJob): NormalizedJob {
  return {
    key: buildJobKey({
...
export async function fetchRemotive(): Promise<RemotiveJob[]> {
...
export function normalizeHimalayas(raw: HimalayasJob): NormalizedJob {
...
export function normalizeJobicy(raw: JobicyJob): NormalizedJob {
 ...
 */
export function normalizeInstahyre(raw: InstahyreJob): NormalizedJob | null {
  const company = raw.employer?.company_name;
  if (!company || !raw.title) return null;
...
export async function fetchInstahyre(): Promise<InstahyreJob[]> {
  ...
  const body = (await res.json()) as { objects?: InstahyreJob[] };
  return body.objects ?? [];
}
