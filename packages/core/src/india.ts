...
export function mentionsIndia(values: string[]): boolean {
...
export function mentionsIndiaStrict(values: string[]): boolean {
  return normalized(values).some((v) => INDIA_STRICT_PATTERNS.some((p) => p.test(v)));
}

export function mentionsWorldwide(values: string[]): boolean {
 ...
 * positive exclusion.
 */
export function isIndiaEligible(job: NormalizedJob): boolean {
...
export function isIndiaLocated(job: NormalizedJob): boolean {
  return (
    mentionsIndia([job.locationRaw]) ||
    mentionsIndia(job.locationRestrictions) ||
 ...
 */
export function isIndiaLocatedStrict(job: NormalizedJob): boolean {
 ...
 */
export function isIndiaExcluded(job: NormalizedJob): boolean {
 ...
 */
export function sortByIndiaPriority<T extends NormalizedJob>(jobs: T[]): T[] {
  const tier = (job: T): number => {
