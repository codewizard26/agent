 ...
 */
export function isEngineeringRole(title) {
    if (NON_ENGINEERING_PATTERNS.some((p) => p.test(title)))
        return false;
 ...
 */
export function deriveRoleFamilies(profile) {
    ...
    if (frontend)
        families.push("frontend developer");
    if (backend)
        families.push("backend developer");
    if (frontend && backend)
        families.push("full stack developer");
    return families;
}
