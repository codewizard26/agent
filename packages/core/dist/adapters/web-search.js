...
function recencyPhrase(timeFrameDays) {
    if (timeFrameDays === null)
        return "";
    if (timeFrameDays <= 1)
        return " posted in the past 24 hours";
 ...
 */
export function buildSearchQueries(profile, timeFrameDays) {
 ...
 */
export async function fetchViaWebSearch(profile, timeFrameDays, client) {
    const queries = buildSearchQueries(profile, timeFrameDays);
    const findings = await client.searchWeb({
        maxSearches: 8,
        prompt: "Search the web for currently-open job postings matching this " +
    ...
    const found = await client.parse({
        ...
        prompt: "Convert these search findings into structured job rows. Use an empty " +
