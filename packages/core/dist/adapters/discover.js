...
export function candidateTokens(companyName) {
 ...
 */
export async function probeBoardToken(provider, token) {
    if (provider === "ashby") {
...
export async function discoverBoards(companyNames, existing, probe = probeBoardToken) {
    ...
    for (const outcome of settled) {
        ...
        if (outcome.status !== "fulfilled" || !outcome.value.hit)
            continue;
