...
export async function createBlueskySession(identifier, appPassword) {
    ...
    if (!res.ok)
        throw new Error(`bluesky auth: HTTP ${res.status}`);
    const body = (await res.json());
    return body.accessJwt;
}
export async function searchBlueskyPosts(accessJwt, query, limit = 50) {
    ...
    if (!res.ok)
        throw new Error(`bluesky search: HTTP ${res.status}`);
    const body = (await res.json());
    return body.posts ?? [];
}
export function buildBlueskyQueries(profile) {
