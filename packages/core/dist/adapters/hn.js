...
export async function findLatestHiringThread() {
    const res = await fetch("https://hn.algolia.com/api/v1/search?tags=story,author_whoishiring&query=Who%20is%20hiring&hitsPerPage=5");
    if (!res.ok)
        throw new Error(`hn search: HTTP ${res.status}`);
    const body = (await res.json());
    const hiring = body.hits
        .filter((h) => /who is hiring/i.test(h.title))
        .sort((a, b) => b.created_at_i - a.created_at_i)[0];
    if (!hiring)
        throw new Error("hn: no who-is-hiring thread found");
    return Number(hiring.objectID);
}
export async function fetchThreadComments(storyId) {
    const res = await fetch(`https://hn.algolia.com/api/v1/search?tags=comment,story_${storyId}&hitsPerPage=1000`);
    if (!res.ok)
        throw new Error(`hn comments: HTTP ${res.status}`);
    const body = (await res.json());
    return body.hits.filter((h) => h.comment_text);
}
export async function parseHnComment(comment, client) {
    const parsed = await client.parse({
        schema: HnJobSchema,
        schemaName: "hn_job",
        tier: "utility",
        maxOutputTokens: 4096,
        prompt: "Extract the job posting from this Hacker News comment. " +
